...
from qf_invert import (
    qf_matrix_inverse,
    float_matrix_to_qfloat_arrays,
    qfloatNsigns_arrays_to_float_matrix,
)
...
class EncryptedMatrixInversion:
    ...
    def __init__(
        self, n, sampler, qf_base=2, qf_len=32, qf_ints=16, trueDivision=False
    ):
        self.shape = (n, n)
        ...
        compiler = fhe.Compiler(
            lambda x, y: qf_matrix_inverse(x, y, params),
    ...
    def quantize(self, matrix: np.ndarray) -> np.ndarray:
        return float_matrix_to_qfloat_arrays(
            matrix, self.qf_len, self.qf_ints, self.qf_base
        )
    ...
    def dequantize(self, quantized_inverted_matrix: np.ndarray) -> np.ndarray:
        return qfloatNsigns_arrays_to_float_matrix(
            quantized_inverted_matrix, self.qf_ints, self.qf_base
        )
...
for name, sampler in {normal_sampler, uniform_sampler}:
    for n in [2, 3, 5, 10]:
        ...
        encrypted_matrix_inversion = EncryptedMatrixInversion(
            n, sampler, qf_base, qf_len, qf_ints, trueDivision
        )
        end = time.time()
